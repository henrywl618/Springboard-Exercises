...
class Boggle():
    ...
    def read_dict(self, dict_path):
        ...
        words = [w.strip() for w in dict_file if len(w.strip()) > 1]
        dict_file.close()
        return words
    ...
    def find_answers(self, board):
        """Returns list of words found on the board """
        return [word for word in self.words if self.find(board,word.upper())]
    
    def find_coord(self, board, word, y, x, coordinates, seen):
        """Get the coordinates for the given word on the board"""

        # pdb.set_trace()
        if x > self.size-1 or y > self.size-1:
            return False

        # This is called recursively to find all instances of characters in the given word.
        ...
        # Base case: we are down to the last letter append coordinates and return

        if len(word) == 1:
            coordinates.append(f"{y}{x}")
            return True
        ...
        if y > 0:
            if self.find_coord(board, word[1:], y - 1, x,coordinates, seen):
                coordinates.append(f"{y}{x}")
                return True
        ...
        if x > 0:
            if self.find_coord(board, word[1:], y, x - 1,coordinates, seen):
                coordinates.append(f"{y}{x}")
                return True
        ...
        # diagonals
        if y > 0 and x > 0:
            if self.find_coord(board, word[1:], y - 1, x - 1,coordinates, seen):
                coordinates.append(f"{y}{x}")
                return True
    ...
    def get_coord(self, board, word, coordinates):
            print(word)
            # pdb.set_trace()
            for y in range(0, self.size):
                for x in range(0, self.size):
                    if len(coordinates) == len(word):
                        return
                    self.find_coord(board, word, y, x, coordinates, seen=set())
